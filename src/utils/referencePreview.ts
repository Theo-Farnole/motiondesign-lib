...
export function getReferencePreviewImageTimecode(
  reference: Pick<MDReference, "timecode" | "previewImageOffset">
) {
  return getReferenceScreenshotTimecode(reference);
}

export function getReferencePreviewVideoStartTimecode(
  reference: Pick<MDReference, "timecode" | "previewVideoOffset">
) {
  const offset = reference.previewVideoOffset ?? 0;
...
export function getReferencePreviewFilename(
  reference: Pick<
    MDReference,
    "videoId" | "timecode" | "previewVideoOffset" | "previewDuration"
  >,
  defaultSeconds = 2.4
) {
  const t = getReferencePreviewVideoStartTimecode(reference);
